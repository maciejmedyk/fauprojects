  ...
  <body>
	...
	<div id="acct_container">
        ...
        <div class="messsidebox">
          ...
          <a href="account-inbox.php"><button id="activebutton" type="button" class="btn btn-default"> Inbox </button><br></a>
        ...
        <?php
          ...
          else
          {
            ...
            $data = mysql_query("SELECT * FROM messages WHERE messageid LIKE '%{$id}%'");
            ...
            {
                $data = mysql_query("UPDATE messages SET unread=1 WHERE messageid LIKE '%{$id}%'");
            }

          }

          $data = mysql_query("SELECT * FROM messages WHERE recipient LIKE '%{$_COOKIE[$cookie_name]}%' ORDER BY timestamp DESC") or die(mysql_error()); 
          
          Print "<table border cellpadding = 3 id=\"messagetable\">";

          if(mysql_num_rows($data) == 0)
          {
              echo "<h4 align=\"center\"> NO MESSAGES </h4>";
          }
          else
          {
          ...
          { 
             if($info['unread'] == "1"){
                ...
                <form action="account-inbox.php" method="post"><td><input class="hidden" name="messageid" value="<?php  echo $id;  ?>"><input type="submit" value="●"></td></form>
                <?php
