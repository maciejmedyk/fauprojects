  ...
  <body>
	...
	<div id="acct_container">
        ...
        <div class="messsidebox">
          ...
          <a href="account-compose.php"><button id="activebutton" type="button" class="btn btn-default"> Compose </button><br></a>
        </div>
        ...
        <div class="messagespacer">
            <h4> Messages - Compose </h4>
        </div>
        <div class="composeside">
        <form class="composeform" action="account-sentmail.php" method="post">
          <?php 
          include('connection.php');
          $user = ($_COOKIE[$cookie_name]);
          $data = mysql_query("SELECT * FROM members WHERE usrlogin LIKE '%{$user}%'") or die(mysql_error()); 

          echo "<label for=\"sender\">Sender</label><br>";
          echo "<select width=\"200px\" type=\"email\" id=\"sender\" name=\"sender\" value=\"" . $_COOKIE[$cookie_name] . "\" required><br>";
          ...
          echo "<label for=\"recipient\">Recipient</label><br>";
          echo "<select width=\"200px\" type=\"email\" id=\"recipient\" name=\"recipient\" required>";
          ...
          else
          {
            ...
            $data = mysql_query("SELECT * FROM messages WHERE messageid LIKE '%{$id}%'") or die(mysql_error());
            
            $info = mysql_fetch_array( $data );
            
            echo "<option value =\"" . $info['sender'] ."\">";
            echo $info['sender'] . "</option>";
          ...
          echo "</select><br><br>"; 
          ?>
          <label for="message">Message</label><br><textarea rows="15" cols="30" type="text" id="message" name="message" required></textarea><br>
          <button type="submit" class="btn btn-default">      Sent      </button><br><br></form>
        </div>
			</div>	
	</div>
		<!-- Footer -->
