...
<html lang="en">
  ...
  <body>
	...
	<div id="acct_container">
        ...
        <form class="composeform" action="listing-update.php" method="post">
          <?php 
          include('connection.php');

          $id = trim($_POST['listingid']);
          $data = mysql_query("SELECT * FROM listings WHERE listingid LIKE '%{$id}%'") or die(mysql_error()); 
          ...
          echo "</select><br><br>"; 
          ?>
          <input class="hidden" type="text" id="listingid" name="listingid" value="<?php echo $info['listingid']; ?>">
          <label for="classid">Class Id</label><br>
          <input type="text" id="classid" name="classid" value="<?php echo $info['classid']; ?>"><br><br>
          <label for="description">Listing</label><br><textarea rows="15" cols="30" type="text" id="description" name="description"><?php echo $info['description']; ?></textarea><br>
          <button type="submit" class="btn btn-default">      Update Listing      </button></form>
          <?php } ?>
        </div>
			</div>	
	</div>
