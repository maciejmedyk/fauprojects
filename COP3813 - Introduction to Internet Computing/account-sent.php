  ...
  <body>
	...
	<button id="loginbutton" type="button" class="btn btn-default" disabled>  Welcome: <text id="loguser"><?php echo $_COOKIE[$cookie_name];?></text></button><br>
        ...
        <div class="messsidebox">
          ...
          <a href="account-sent.php"><button id="activebutton" type="button" class="btn btn-default"> Sent </button><br></a>
        ...
        <?php
          ...
          $data = mysql_query("SELECT * FROM messages WHERE sender LIKE '%{$_COOKIE[$cookie_name]}%' ORDER BY timestamp DESC") or die(mysql_error()); 
          
          Print "<table border cellpadding = 3 id=\"messagetable\">";
          if(mysql_num_rows($data) == 0)
          {
              echo "<h4 align=\"center\"> NO SENT MESSAGES </h4>";
          }
          else
          {
            Print "<th>Recipient</th>" . "<th width=100%>Message</th>" . "<th>Date and Time</th>" . "<th> </th>";
          }
          while($info = mysql_fetch_array( $data )) 
          { 
             if($info['unread'] == "1"){
                Print "<tr>";
                //Print "<td>".$info['sender'] . "</td>"; 
